...
class TMDbService
{
//    https://github.com/vuphuong87/imdb-php-api/blob/master/src/TMDb.php
    const POST = 'post';
    const GET = 'get';
    const HEAD = 'head';
     ...
     * @return void
     */
    public function __construct( $default_lang = 'en', $config = FALSE, $scheme = TMDbService::API_SCHEME,ParameterBagInterface $params)
    {
        $this->_apikey = $params->get('TDBM_API_KEY');
        $this->_apischeme = ($scheme == TMDbService::API_SCHEME) ? TMDbService::API_SCHEME : TMDbService::API_SCHEME_SSL;
    ...
    public function getAuthToken()
    {
        ...
        if( ! isset($result['request_token']))
        {
            ...
            {
                throw new \Exception('No valid request token from TMDb');
            }
            else
            {
                return FALSE;
    ...
    public function addFavoriteMovie($account_id, $session_id = NULL, $movie_id = 0, $favorite = TRUE)
    {
        ...
        return $this->_makeCall('account/'.$account_id.'/favorite', $params, $session_id, TMDbService::POST);
    }
    ...
    public function addMovieToWatchlist($account_id, $session_id = NULL, $movie_id = 0, $watchlist = TRUE)
    {
        ...
        return $this->_makeCall('account/'.$account_id.'/movie_watchlist', $params, $session_id, TMDbService::POST);
    }
    ...
    public function addMovieRating($session_id = NULL, $movie_id = 0, $value = 0)
    {
        ...
        return $this->_makeCall('movie/'.$movie_id.'/rating', $params, $session_id, TMDbService::POST);
    }
     ...
     * @param const $imagetype			Image type: TMDbService::IMAGE_BACKDROP, TMDbService::IMAGE_POSTER, TMDbService::IMAGE_PROFILE
    ...
    public function getImageUrl($filepath, $imagetype, $size)
    {
        ...
        if(isset($config['images']))
        {
            ...
            {
                throw new \Exception('The size "'.$size.'" is not supported by TMDb');
            }
        }
        else
        {
            throw new \Exception('No configuration available for image URL generation');
        }
    }
    ...
    public function getAvailableImageSizes($imagetype)
    {
        ...
        {
            throw new \Exception('No configuration available to retrieve available image sizes');
        }
    }
    ...
    {
        $headers = $this->_makeCall($uri, NULL, NULL, TMDbService::HEAD);
     ...
     * @param const $method				TMDbService::GET or TMDb:POST (default TMDbService::GET)
     * @return TMDb result array
     */
    private function _makeCall($function, $params = NULL, $session_id = NULL, $method = TMDbService::GET)
    {
        ...
        $url = $this->_apischeme.TMDbService::API_URL.'/'.TMDbService::API_VERSION.'/'.$function.'?'.http_build_query($auth_array, '', '&');

        if($method === TMDbService::GET)
        {
        ...
        if (extension_loaded('curl'))
        {
            ...
            elseif($method == TMDbService::HEAD)
            {
            ...
            if($error_number > 0)
            {
                throw new \Exception('Method failed: '.$function.' - '.$error_message);
            }
